...
const Reg = () => {
  const [title,setTitle]=useState<string>("");const [des,setDes]=useState<string>("");
  ...
  const onConfirm=async()=>{
    if(title&&des&&addr&&number&&name&&state&&uname&&pass&&pass1){
      const id=Buffer.from(title+name+number+uname).toString('base64');
      setOpenS(true);
      ...
      await axios.post('https://thevalley-qzqc.shuttle.app/addsales',myWeb)
    ...
    <Head><title>New Product</title></Head>
      <Container>
        ...
        <Paper sx={{padding:5,marginTop:5,marginBottom:3}}>
          ...
          />
            <Grid container sx={{ display: 'flex', alignItems: 'center',marginY:5 }} spacing={2}>

              {img.map((x:any)=><Grid item xs={6} md={1.8}><img style={{width: 140,height: 140,marginRight: 2,borderRadius: 5}} src={x} alt='Product Pictures' /></Grid>)}
              <Grid item>
                ...
                <Button color='error' variant='outlined' onClick={() =>{setImg([]);setLimErr(false)}}>
                  Reset
                </Button>
                <Typography variant='body2' sx={{ marginTop: 5 }}>
                  Allowed PNG or JPEG. Max size of 800K. Max number of 4.
                </Typography>
                {limErr&&<Typography variant='body2' color="error" sx={{ marginTop: 5 }}>Max number of images reached</Typography>}
              </Grid>
            </Grid>
          <Alert>
          ...
          <Stack sx={{width:"100%"}} direction="row" alignItems="stretch" justifyContent="space-between" mb={5} p={1} alignSelf="flex-start">
              ...
              <TextField
                ...
                error={!(pass1===pass)}
                helperText={!(pass1===pass)?"Passwords doesnt match":"Confirm your password."}
              />
          </Stack>
          </Paper>
          ...
          <Stack mt={10} alignItems="center" justifyContent="flex-start">
            <Button sx={{width:'60%'}} variant="contained" onClick={onConfirm}>Submit Product</Button>
          </Stack>
        </Paper>
        <Dialog
        open={open}
